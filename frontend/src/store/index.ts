...
export const store = createStore<State>({
  ...
  mutations: {
    ...
    removeDismissedIssue(state: State, value: string) {
      state.dismissedIssues = state.dismissedIssues.filter(
        (v: string) => v !== value,
      );
    },
    ...
    removeSelectedNode(state: State, id: string) {
      state.selectedNode = state.selectedNode.filter(
        (node: any) => node.id !== id,
      );
    },
    addCustomIssue(state: State, cIssue: customIssue) {
      const idx = state.issues.findIndex(
        (issue: issueSpec) => issue.type === cIssue.gType,
      );
      if (idx === -1) return;
  ...
  actions: {
    async updateJson({ commit, state }, { formData, editorValue, type }) {
      // 上传一份新数据，需要清空store中的状态
      if (type === 'upload') {
        store.commit('adjustState');
      }
  ...
  getters: {
    detailedIssue(state) {
      // 所有issues包括已经dismiss的
      const TreeIssues: Array<Array<issueNode>> = state.issues.map(
        (issue: issueSpec, i: number) => issue.details.map((specificIssue: string, j: number) => ({
          label: specificIssue,
          id: specificIssue,
          children: issue.description[j].map(
            (desc: Array<string>, k: number) => ({
    ...
    activeIssue(state, getters) {
      ...
      if (filterOutDismissed.length > 0) {
        state.selectedIssue = filterOutDismissed[0].children?.[0].id as string;
      }
    ...
    inactiveIssue(state, getters) {
      ...
      flattenTree.forEach((node: issueNode) => {
        if (!node.children || !node.children.length) return;
        const filtered = node.children.filter((c: issueNode) => state.dismissedIssues.includes(c.id));
        if (!filtered.length) return;
    ...
    getCliqueData(state) {
      const issueWithLoc = state.issues.map((issue: issueSpec, i: number) => issue.details.map((value: string, j: number) => ({
      ...
      const flattenedSpecific: any[] = issueWithLoc.reduce(
        (a, b) => [...a, ...b],
        [],
      );
    ...
    KVInfo(state, getters) {
      ...
      data.forEach(({ specificIssue, generalType }) => {
        let col = 0;
        (Object.keys(IssueByKVS) as Array<keyof typeof IssueByKVS>).forEach(
          (k) => {
        ...
        const row = generalTypeMapRowIndex.findIndex(
      ...
      const stackInfo = generalTypeMapRowIndex.map((k, i) => ({
        stackName: k,
        stackData: matrix[i].map((item: number) => ({
          value: item,
    ...
    categoryInfo(state) {
      const piedataWithInfo = state.issues.map((d: issueSpec, i: number) => {
        ...
        return {
          name: issueTypeMapping[d.type],
          value: d.count - filterOutDismissed,
          details: d.details,
        };
      });
      const accordingColor = state.issues.map(
        (d: issueSpec) => OverviewLegendFill[d.type],
      );
      return {
        piedataWithInfo,
        accordingColor,
    ...
    prepareSelectedIssue(state) {
      // 此处应该直接返回echarts option，但是存在d3-boxplot的问题，暂时先不管
      ...
      if (!Array.isArray(source)) {
        ...
        const config = {
          ...
          titleM: generateDescription(
            state.issues[i].description[j][k],
            state.issues[i].details[j],
            'rich',
          ),
        };
        if (
          specificIssueTypes.dataAssociations === state.issues[i].details[j]
        ) {
      ...
      locs.forEach((idx: number) => {
        ...
        nodeInfo.type.forEach((type: string, index: number) => {
          ...
          const info = {
            ...
            headInfo: nodeInfo.key ? nodeInfo.key : 'Unnamed Node',
          };
          switch (type) {
            case 'n': {
              const { freqHistoData } = dataPrepForNum(info);
              optionArray.push(
                generateFreqHistoOption(freqHistoData, 'detail'),
              );
              break;
            }
            case 's': {
              ...
              optionArray.push(
                generateFreqHistoOption(strLengthFrequency, 'detail'),
              );
              optionArray.push(
                generateBarOption(categoryFrequencyData, 'detail'),
              );
              break;
            }
            case 'b': {
    ...
    abledContextMenu(state) {
      const menuList = Object.keys(IssueBySixCategory) as Array<
      keyof typeof IssueBySixCategory
      >;
    ...
    generateBubbleData(state, getters) {
      ...
      existingNode.forEach((nodeInfo: any) => {
        ...
        bubbleData.push({
          ...
          y:
            y
            + (showAttr && attrs.length ? -attributeViewStyle.height : 0)
            - typeNodeStyle.nodeHeight / 2,
          width: Math.max(
            attrs.length * attributeViewStyle.width,
            typeNodeStyle.nodeWidth,
          ),
          height:
            (showAttr && attrs.length ? attributeViewStyle.height : 0)
      ...
      const pathData = cliqueData.map((clique: cliqueInfo) => {
        ...
        const cliqueIssueName = `${clique.specificIssue.split(' ')
          .join('+')}`;
        const nodeInClqiue = bubbleData.filter((d: nodeBubbleDesc) => clique.location.includes(parseInt(d.id, 10)));
